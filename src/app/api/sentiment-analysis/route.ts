...
async function analyzeLocation(location: { lat: number; lng: number; sentimentBias?: number }) {
  // For testing, use the sentimentBias if provided, or generate a random sentiment
  const baseSentiment = location.sentimentBias !== undefined 
    ? location.sentimentBias 
    : Math.random() * 2 - 1;
    
  // Add some random variation to the base sentiment
  const sentiment = Math.max(-1, Math.min(1, baseSentiment + (Math.random() - 0.5) * 0.4));
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to analyze sentiments' }, { status: 500 });
  }
}
