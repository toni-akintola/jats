...
export class RiskService {
  ...
  private async getFemaRegion(state: string): Promise<FemaRegion> {
    ...
    const region = data.FemaRegions.find(
      (r: FemaRegionResponse["FemaRegions"][0]) =>
        // console.log("r is : ", r)
  ...
  private async getDisasterDeclarations(
    state: string,
    county: string,
    ...
    return data.DisasterDeclarationsSummaries.map(
      (d: FemaDisasterResponse["DisasterDeclarationsSummaries"][0]) => ({
  ...
  private calculateRiskScore(disasters: DisasterDeclaration[]): number {
    ...
    const weightedSum = disasters.reduce((sum, disaster) => {
      const yearDiff =
        currentYear - new Date(disaster.declarationDate).getFullYear();
  ...
  private calculateHistoricalTrends(
    disasters: DisasterDeclaration[],
  ): { year: number; disasterCount: number }[] {
    const countsByYear = disasters.reduce(
      (acc, disaster) => {
  ...
  public async assessRisk(address: string): Promise<RiskAssessment> {
    ...
    const disasters = await this.getDisasterDeclarations(
      location.state,
      location.county,
    );

    // Calculate disaster type distribution
    const disastersByType = disasters.reduce(
      (acc, disaster) => {
